...
class SamplingBasedPathPlanner():
    ...
    def RRTStar(self, RRT_algorithm, environment, bounds, start_pose, goal_region, object_radius, steer_distance, distance_travelled, num_iterations, resolution=3, runForFullIterations=True, drawResults=False):
        ...
        if self.RRTFamilySolver.path and drawResults:
            ...
            self.RRTFamilySolver.path.append((goal_region.centroid.x, goal_region.centroid.y))
...
class RRTFamilyPathPlanner():
    ...
    def initialise(self, environment, bounds, start_pose, goal_region, object_radius, steer_distance, num_iterations, resolution):
        ...
        self.sample_space = []
        for lanes in environment[-1]:
            self.sample_space.append(lanes.shapely_object)
        self.obstacles = environment[0 : -1]
    ...
    def RRTStarSearch(self):
        ...
        for i in range(self.N):
            ...
            if self.isEdgeCollisionFree(nearest_point, new_point):
                ...
                if (i%self.check_interval == 0 and self.path_found):# or self.in_goal_region: #self.isAtGoalRegion(new_point):
        ...
        for i in range(self.N):
            ...
            if free_collision:
                ...
                if (self.path_found == False):
                    ...
                    if (at_goal_region):# or self.in_goal_region:
                        self.path_found = True
                if (self.path_found):
                    at_goal_region = self.isAtGoalRegion(new_point)
                    if (at_goal_region):# or self.in_goal_region:
    ...
    ##################################################################
    ########## generate random point inside the lane #################
    def between_lines(self, x, y):
        if STRtree(self.sample_space).query(Point(x,y)):
            return True
        return False
    ##################################################################
    ...
    def isAtGoalRegion(self, point):
        ...
        # stop if the robot arrives in goal region
        self.in_goal_region = self.goal_region.contains(Point(point))
        if self.in_goal_region:
            print("In goal region!")
        return inGoal >= 0.5
